class Game{
	...
	loadAssets(){
		...
		loader.load( './assets/rc_time_trial1.fbx', 
		function ( object ){
	...
	initPhysics(){
		...
		world.broadphase = new CANNON.SAPBroadphase(world); //set collision detection algorithm
		...
		const wheelBodies = [];
		let index = 0;
		const wheels = [this.car.wheel[0]];
        this.car.wheel[0].visible = true;
        this.car.wheel[0].castShadow = true;
		for(let i=0; i<3; i++){
			let wheel = this.car.wheel[0].clone();
		...
		world.addEventListener('postStep', function(){
			...
			game.vehicle.wheelInfos.forEach(function(wheel){
                ...
                wheelBodies[index].threemesh.quaternion.copy(q1);
				index++; 
			});
		});
	...
	createColliders(){
		...
		game.colliders.forEach(function(child){
